...
lotr_tidy %>% 
  ...
  spread("Race_Gender", value = "Words") 
...
`unite` # pastes 2 categories together into 1 and `separate`.

```{r}

```

`uncount` # makes a repeated row for the # of observations you have (as the opposite of `dplyr::count()`)
...
`fill` # fills in all NAs with data value above
