...
```{r}
(1:10) * (1:2) # recycling: shorter vector is recycled so that it is of equal length (ie. 1, 2, 1, 2, 1, 2)
```
...
x <- character(0) # initiated a character vector of length 0 

for(i in 1:length(wesanderson)){
  x[i] <- wesanderson[[i]][1] # ith component of wesanderson and we want the 1st element of that component
}
x
```
...
```{r}
str(got_chars, list.len=4, max.level=1) # only shows the 1st 4 of the list
# max.level = 1 to view 1st level of list (the people)
# max.level = 2 to view sublists (info. of ppl)
...
```{r}
map_chr(x, str_c, "potato.", sep="-") # specified other arguments "potato." and sep="-"
```
...
map_chr(wesanderson, function(x){ x = x[1] }) # use map_chr because we want vector not list

# Method 2: head
map_chr(wesanderson, head, n = 1) # n if positive or zero, size for the resulting object: number of elements for a vector (including lists)

# Method 3:
map_chr(wesanderson, `[`, 1)  # subsetting wesanderson to the first element
# x[1] is interchangable with `[`(x,1)
```
...
```{r}
# subsetting by index 1
map_chr(wesanderson, 1) # %>% unname() # unname() means names of function will dissappear
```
...
- Is a vector output appropriate here? NO! because we don't know how long each component is going to be.
- Use a pipe.
...
map(got_chars, `[`, desired_info) %>% head() # ea person only has a list with 2 components "name" and "born"

map_dfr(got_chars, `[`, desired_info) # data frame where ea person has their own row
```
...
```{r}
got_chars %>% {
    tibble( # tibble function allows us to manually construct a df
        name = map_chr(., "name"), # . refers to entire got_chars data # subsetting to name and converting to character vector. Results in character vector of names
        born = map_chr(., "born")
    )
}
...
```{r}
got_chars %>% {
    . # putting got_chars as first line of {
    tibble(
        name = map_chr(., "name"),
        born = map_chr(., "born")
    )
