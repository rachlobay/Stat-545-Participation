...
predict(my_lm, newdata = tibble(gdpPercap = c(500,600,700))) # new data arugment is what x values you want to make predictions on (in the form of a data frame)
```
...
```{r}
tidy(my_lm) # gives estimates, std.error, statistic and p.value 
# p-value: logGDP per cap has a significant linear effect on lifeExp

augment(my_lm) # gives column of fitted values --> compare estimates to actual
# can specify data for argument function
augment(my_lm, newdata = tibble(gdpPercap = c(400,500,600))) # doesn't include true y values, but igives us estimates of y values

glance(my_lm) # tells us how well model fits to data
```
