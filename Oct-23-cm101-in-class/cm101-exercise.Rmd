...
max_minus_min(runif(100)) # generating a bunch of numbers close to 100
```
...
expect_identical(0.1 + 0.2, 0.3) # not identical because floating point error
```
...
test_that("Simple cases work", {
    ...
    expect_lt(max_minus_min(runif(100)),1) # Is returned value less than 1?
}
)
```
...
```{r}
max_minus_min(gapminder[c('lifeExp', 'gdpPercap', 'pop')]) # taking max - min of all possible values in data frame
...
```{r}
mmm1 <- function(x) {
    stopifnot(!is.numeric(x)) # if not numeric, stop
    max(x) - min(x)
}
...
```{r}
# by default, qd2 takes probs argument to be a vector of 0,1 (max-min)
...
v <- rnorm(100)
qd2(v, probs=c(0,1), na.rm=FALSE, type = 1) # assume 1 corresponds to probs
```