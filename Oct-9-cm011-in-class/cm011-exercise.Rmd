...
```{r}
a <- 3
print(typeof(a)) # gives the "type" of object from R's point of view
print(mode(a)) # gives the "type" of object from the point of view of S implementation
print(class(a)) # identify what "type" an object is from the point of view of object-oriented programming in R
```
...
# vector of length 0
seq_len(0)
seq_len(10) # makes a vector from 1 - 10
```
...
# Multiply each component by 1 through its length (ie. first entry times 1, 2nd entry times 2, ... last entry times 10 
...
```{r}
a * 1:3 # R prints smaller length vector over until end of longer vector length
# warning for above, because smaller vector duplicate chopped off because otherwise it won't match up
a * 1:2
```
...
```{r}
a > 0 & a < 5
a > 0 && a < 5 # not vectorized
# && only compares the first entry of both vectors
any(a > 0 & a < 5)
all(a > 0 & a < 5)
```
...
# functions in R like the setNames function don't change objects (ie. setNames doesn't change the course object)
course
```
...
course[["section"]] # [[]] only allows you to return a signle entry and only returns object (not header name)
```
...
Unlike vectors, which are atomic/homogeneous, a list in R is heterogeneous (ie. lists can hold basically anything)

Try storing the course code (`"STAT"` and `545`) again, but this time in a list. Use the `list()` function.

```{r}
(course <- list("STAT", 545)) # 545 is in original form --> not made into "545"
```
...
```{r}
my_list[2] # list of length 1 contains a vector of length 2
my_list[[2]] # entry of that car (gets rid of list structure)
```
...
my_list["year"]
my_list[["year"]]
my_list$year # like [[]], returns entry of that car
```

Smells a little like `data.frame`s. It turns out a `data.frame` is a special type of list
# each component of a data frame must be of same length
...
# Matrix:
diag(5) 
# matrices do not allow us to hold data of different types (1 type of data in matrix)
```

Note that there's a difference between a list of one object, and that object itself! This is different from vectors.

```{r}
# are the following identical objects?
identical(list(4), 4)
identical(c(4), 4) # vectors don't have the extra baggage like lists do
```
